Shader "Custom/WaterShore" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
			...
			// Get foam effect
			float foam = Foam(shore, IN.worldPos.xz, _MainTex); // Defined in "WaterInclude.cginc" include file

			// Get waves effect
			float waves = Waves(IN.worldPos.xz, _MainTex);		// Defined in "WaterInclude.cginc" include file
			waves *= 1 - shore; // Decrease waves when close to shore

			// Combine both effects
			fixed4 c = saturate(_Color + max(foam, waves)); // The closer to shore the brighter the color
